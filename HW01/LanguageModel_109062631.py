...
def add1Smooth(bi_gram, uni_count, bi_count):
    V = len(list(Counter(uni_count.keys())))
...
def sentenceProb(sentence, uni_count, bi_count):
    bi_gram = biGramTokenize(sentence)
    sent_prob = 0.0
    for i in bi_gram:
        sent_prob = sent_prob + math.log(add1Smooth(i, uni_count, bi_count), 10)
    return sent_prob


# The correct answer is P(sent1) < P(sent2) and P(sent3) < P(sent4)
if __name__ == "__main__":
    # Count # of times which 1-grams(unigram) and 2-grams(bigram) appeared within full text
    ...
    print("%s (%.3f)"%(sent1, sentenceProb(sent1, uni_count, bi_count)))
    print("%s (%.3f)"%(sent2, sentenceProb(sent2, uni_count, bi_count)))
    print("%s (%.3f)"%(sent3, sentenceProb(sent3, uni_count, bi_count)))
    print("%s (%.3f)"%(sent4, sentenceProb(sent4, uni_count, bi_count)))
